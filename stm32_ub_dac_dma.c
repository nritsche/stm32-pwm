...
//--------------------------------------------------------------
// stellt eine hardgecodete Kurvenform für DAC1 ein
//--------------------------------------------------------------
...
//--------------------------------------------------------------
// stellt eine eigene Kurvenform für DAC1 ein
//--------------------------------------------------------------
void UB_DAC_DMA_SetWaveform1_man(uint16_t* wave, uint16_t len)
{
  DMA_InitTypeDef DMA_InitStructure;
  ...
  DMA_InitStructure.DMA_Channel = DMA_Channel_7;
  DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)DAC1_DHR_12R1_ADDRESS;
  DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)wave;
  DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
...
//--------------------------------------------------------------
// stellt eine Kurvenform f�r DAC2 ein
//--------------------------------------------------------------
...
//--------------------------------------------------------------
// stellt eine eigene Kurvenform für DAC2 ein
//--------------------------------------------------------------
void UB_DAC_DMA_SetWaveform2_man(uint16_t* wave, uint16_t len)
{
  DMA_InitTypeDef DMA_InitStructure;
...
//--------------------------------------------------------------
// stellt die Frequenz f�r DAC2 ein
// Grundfrequenz = 2*APB1 (APB1=42MHz) => TIM_CLK=84MHz
// vorteiler : 0 bis 0xFFFF
// periode   : 0 bis 0xFFFF
//
// Ausgangs-Frq = TIM_CLK/(vorteiler+1)/(periode+1)/wave_len
//--------------------------------------------------------------
void UB_DAC_DMA_SetFrq2(uint16_t prescaler, uint16_t overflow)
{
  TIM_TimeBaseInitTypeDef    TIM_TimeBaseStructure;
